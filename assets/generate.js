...
(function(){
  ...
  "sentences": [
    {"en": "英文句子（保持原文，逐句拆開；**不要**把說話者名字寫進這裡）", "zh": "繁體中文翻譯", "ana": "給初中生看的詳細講解，3-5句：①整句在說什麼 ②句子結構怎麼搭（主謂賓/時態/從句）③關鍵詞的詞性和用法 ④為什麼這樣用、換個說法行不行 ⑤容易錯的地方。白話、可用 <b>標籤</b>，重點前加 ⭐；不要只寫一行",
  ...
  "sentences": [
    {"jp": "日文句子，漢字必須標振假名，格式為 漢字[かな]，如 私[わたし]は 学生[がくせい]です。（只在漢字後面用方括號標讀音，假名/片假名/數字不要標）",
     ...
     "ana": "給初中生看的詳細講解，3-5句：①整句意思 ②每個助詞(は/が/を/に…)的作用 ③句型/活用怎麼變 ④為什麼這樣用 ⑤容易錯的地方。白話、可用 <b>標籤</b>，重點前加 ⭐；不要只寫一行",
  ...
  function systemPrompt(lang){
    ...
    return `你是一位耐心的${langName}精讀老師，為初中生製作精讀學習卡片。
用戶會給你一段${langName}課文（可能是圖片或文字）。請把它做成精讀數據，嚴格按下面的 JSON 結構輸出。

規則：
1. **只輸出 JSON**，不要任何解釋、不要 markdown 代碼框。
2. 講解（ana / grammar）一律用**繁體中文**，語氣親切、給初中生看，白話講清楚，重點前加 ⭐。
...
${schema}`;
  ...
  function wordsSystemPrompt(lang){
    const schema = lang==='jp' ? SCHEMA_JP : SCHEMA_EN;
    const langName = lang==='jp' ? '日語' : '英語';
    return `你是一位耐心的${langName}老師，為初中生做「單詞精讀課」。
用戶會給你一組${langName}單詞（可能每個詞後面帶中文意思）。請把這組單詞做成精讀數據，嚴格按下面 JSON 結構輸出。

規則：
1. **只輸出 JSON**，不要任何解釋、不要 markdown 代碼框。
2. 講解（ana / grammar）一律用**繁體中文**，語氣親切，重點前加 ⭐。
3. **為每個單詞造一個例句放進 sentences**：句子要**簡單、常用、初中生水平、地道**，能幫孩子學會這個詞怎麼用。每個 sentences 對象：${lang==='jp'?'jp=例句（漢字標振假名 漢字[かな]）、romaji、zh=翻譯、ana=用 2-3 句講這個詞怎麼用':'en=例句、zh=翻譯、ana=用 2-3 句講這個詞怎麼用'}。sentences 的順序與 vocab 一一對應。
...
${schema}`;
  ...
  /* 生詞中文意思二次核對：弱模型偶爾給錯/張冠李戴的中文意思，初中生當事實背最危險。
  ...
  async function verifyVocab(lang, d, onProgress){
    ...
    try{
      const content = await callApi(getTextModel(), [
        { role:'user', content:
          '下面是一份給初中生的'+langName+'生詞表，每個詞附「目前中文意思」。請逐個檢查中文意思對不對：\n'+
          '· 正確或基本可接受 → **原樣返回**，不要改寫措辭。\n'+
          '· **明確錯誤/張冠李戴**（意思根本不是這個詞的）→ 返回正確、簡短、初中程度的中文意思。\n'+
  ...
  async function judgeSentence(lang, word, sentence){
    ...
    const content = await callApi(getTextModel(), [
      { role:'user', content:
        '你是親切的初中'+langName+'老師。孩子用指定單詞造了一個句子，請判斷：\n'+
        '1. 是否用上了指定單詞（複數、過去式、活用等詞形變化都算用上）。沒用上才算沒用上。\n'+
        '2. 句子有沒有**明確的文法錯誤**。**只有下列這種「老師一定會扣分」的清楚錯誤才判 ok:false**：\n'+
        '   時態明顯錯、主謂不一致（第三人稱單數漏 s）、詞形錯（助動詞後沒用動詞原形、'+(lang==='jp'?'助詞用錯、活用形明顯錯':'該用動詞卻用名詞')+'）、句子結構殘缺（缺主詞或動詞讀不通）。\n'+
        ...
        '若真的 ok:false：\n'+
        '  · tip 用繁體中文說清楚**錯在哪＋該怎麼改**（先指出錯處，再說怎麼改對，例如「時態錯了：句子講昨天的事，go 要改成過去式 went」），50字內、初中生看得懂。\n'+
  ...
  async function storyFromWords(lang, words, onProgress){
    ...
    const content = await callApi(getTextModel(), [
      { role:'user', content:
        '請為初中生寫一個非常短的'+langName+'小故事（4-6 句，'+(lang==='jp'?'60 字':'60 詞')+'以內），'+
  ...
  async function exampleFor(lang, word, zh){
    ...
    const content = await callApi(getTextModel(), [
      { role:'user', content:
        '請為初中生用'+langName+'單詞「'+word+'」（中文意思：'+zh+'）造一個**新的、簡單的**例句'+
